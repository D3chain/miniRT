...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/17 20:09:19 by fox               #+#    #+#             */
/*   Updated: 2026/01/17 20:45:51 by fox              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_viewport(struct s_app *app, struct s_camera *camera)
{
	...
	camera->wp.height = FLT_2 * tan(camera->fov_rad / FLT_2) * camera->focal_length;
	...
	if (camera->dir.y > dir_ceil || camera->dir.y < -dir_ceil)
		...
		camera->wp.u = fmul3(normalize3(cross3(camera->dir, (t_real3){FLT_0, -FLT_1, FLT_0})), camera->wp.width);
	...
	camera->wp.upper_left = minus3(plus3(camera->focal_center, camera->dir), plus3(fmul3(camera->wp.u, FLT_0_5), fmul3(camera->wp.v, FLT_0_5)));
	camera->wp.pixel00_loc = plus3(plus3(camera->wp.upper_left, fmul3(camera->wp.u_px, FLT_0_5)),
