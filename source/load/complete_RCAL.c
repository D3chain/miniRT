...
/*   Updated: 2026/01/15 14:17:09 by cgajean          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	complete_C(struct s_app *app, struct s_camera *camera, bool setup_pan_speed)
{
	...
	camera->viewport_width = camera->viewport_height * app->mlx.screen.img_ratio;
	...
	camera->viewport_u_px = fmul3(camera->viewport_u, 1.0 / app->mlx.screen.resolution.x);
	camera->viewport_v = fmul3(normalize3(cross3(camera->dir, camera->viewport_u)), -camera->viewport_height);
	camera->viewport_v_px = fmul3(camera->viewport_v, 1.0 / app->mlx.screen.resolution.y);
	...
	if (setup_pan_speed)
	{
		app->scene.camera.base_pan_speed = ZOOM_BASE_PAN_SPEED;
		app->scene.camera.fov_reference = ZOOM_FOV_REFERENCE;
		app->scene.camera.pan_fov_ratio = app->scene.camera.fov / ZOOM_FOV_REFERENCE;
		app->scene.camera.pan_speed = ZOOM_BASE_PAN_SPEED * app->scene.camera.pan_fov_ratio;
	}
}
