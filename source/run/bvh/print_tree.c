...
/*   Updated: 2026/01/10 11:37:53 by fox              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_tree(t_bvh_base *root, int depth)
{
	if (root->type == NODE_BOX)
	{
		...
		print_double3(((t_bvh_node *)root)->left->bbox.beg, "\t\tdeb: ");
		print_double3(((t_bvh_node *)root)->left->bbox.end, "\t\tend: ");
		
		
		printf("\tRight : %s\n", ((t_bvh_node *)root)->right->type == NODE_BOX ? "NODE_BOX" : "ELEM_BOX");
		print_double3(((t_bvh_node *)root)->right->bbox.beg, "\t\tdeb: ");
		print_double3(((t_bvh_node *)root)->right->bbox.end, "\t\tend: ");		
	...
	else
	{
		...
		for (int j = 0; j < ((t_bvh_elem_box*)root)->n_elems; j++)
		{
			...
			print_double3(((t_bvh_elem_box*)root)->elems[j].u.any.coord, "coord: ");
			print_double3(((t_bvh_node *)root)->bbox.beg, "\t\tdeb: ");
			print_double3(((t_bvh_node *)root)->bbox.end, "\t\tend: ");				
		}
	}
}
